...
export class AuthLogicService {
  ...
  private currentUserSubject = new BehaviorSubject<UserInterface | null>(
  ...
  public login(authData: AuthificationDataInterface): Observable<string> {
    return this.authApiService.login(authData).pipe(
      tap((token: string) => {
        const cleanToken =
  ...
  public register(registerData: RegisterNewUserInterface): Observable<string> {
    return this.authApiService.register(registerData).pipe(
      switchMap((userId) => {
        console.log(
          'Пользователь зарегистрирован с идентификатором Id:',
          userId
        );
        return this.login({
          ...
          tap(() => {
            this.currentUserSubject.next({
              ...
              console.log(
                'Автоматическая авторизация после регистрации завершена успешно!'
              );
          }),
          catchError((error) => {
